...
void CSysInfo::Init()
{
	...
	if((m_hmIphlpapiDll=::LoadLibrary(_T("iphlpapi.dll")))!=NULL)
	{
		...
		//		LPCTSTR lpProcName   // function name
		// );
		m_bIphlpapiDll=TRUE;
...
LPCTSTR CSysInfo::GetAPIAdapterDescr(int i)
{
	if ((m_nNumAdaptersIphlpapi>0) && (m_nNumAdaptersIphlpapi>i))
	{
		...
		if (tmp_pai==NULL)
			...
			{ return (LPCTSTR)tmp_pai->Description; };
	}
	else
		return NULL;
}

LPCTSTR CSysInfo::GetAPIAdapterName(int i)
{
	if ((m_nNumAdaptersIphlpapi>0) && (m_nNumAdaptersIphlpapi>i))
	{
		// Get pointer to the IP_ADDR_STRING structure of requested adapter
		...
		if (tmp_pai==NULL)
			...
			{ return (LPCTSTR)tmp_pai->AdapterName; };
	}
	else
		return NULL;
}
...
//
LPCTSTR CSysInfo::GetAPIAdapterIPStr(int i,int indexIP)
{
	if ((m_nNumAdaptersIphlpapi>0) && (m_nNumAdaptersIphlpapi>i))
	{
		// Get pointer to the IP_ADDR_STRING structure of requested adapter
		...
		if (tmp_pai!=NULL)
		{
			...
			if ((a>0) && (a>indexIP))
			{
				...
				if (pip==NULL)
					...
					return (LPCTSTR)pip->IpAddress.String;
			}
			else
				return NULL;
		}
...
//							indexIP - zero-based index of IP address of network adapter
//
LPCTSTR CSysInfo::GetAPIAdapterMaskStr(int i, int indexIP)
{
	if ((m_nNumAdaptersIphlpapi>0) && (m_nNumAdaptersIphlpapi>i))
	{
		// Get pointer to the IP_ADDR_STRING structure of requested adapter
		...
		if (tmp_pai!=NULL)
		{
			...
			if ((a>0) && (a>indexIP))
			{
				...
				if (pip==NULL)
					...
					return (LPCTSTR)pip->IpMask.String;
			}
			else
				return NULL;
		}
...
LPCTSTR CSysInfo::GetMIBAdapterDescr(int i)
{
	...
	return (LPCTSTR)&pInfo->Description[0];
}
