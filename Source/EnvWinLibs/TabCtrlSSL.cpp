...
void CTabCtrlSSL::OnSize(UINT nType, int cx, int cy)
   {
   ...
   if ( m_resizable )
      ...
      int x, y, nWidth, nHeight, nTabWidth=0, nTabHeight=0;
      DWORD dwStyle = GetStyle ();

      if (dwStyle & TCS_VERTICAL) // Vertical tabs
         {
         nTabWidth = rect.Width () * GetRowCount ();
         x = (dwStyle & TCS_RIGHT) ? 4 : nTabWidth + 4;
         y = 4;
         nWidth = cx - nTabWidth - 8;
         nHeight = cy - 8;
         ...
         { // Horizontal tabs
         nTabHeight = rect.Height () * GetRowCount ();
         //x = 4;
         //y = (dwStyle & TCS_BOTTOM) ? 4 : nTabHeight + 4;
         nWidth = cx - 8;
         nHeight = cy - nTabHeight - 8;
      ...
      int childCount = GetSSLPageCount();

      for ( int i=0; i < childCount; i++ )
         {
         CWnd *pPage = GetSSLPage( i );

         pPage->MoveWindow( 2, nTabHeight+2, cx-nTabWidth-4, cy-nTabHeight-4, TRUE );
         }      
      }
   }
