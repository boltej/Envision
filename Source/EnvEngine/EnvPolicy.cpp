...
*/
// EnvPolicy.cpp: implementation of the EnvPolicy class.
//
//////////////////////////////////////////////////////////////////////
...
OutcomeInfo::OutcomeInfo( EnvPolicy *_pPolicy, LPCTSTR _field, VData _value, int _delay, OutcomeDirective *pDirective ) 
   : pPolicy( _pPolicy )
   , value( _value )
   , col( -1 )
   , m_pMapExpr( NULL )
...
PolicyConstraint::PolicyConstraint( EnvPolicy *pPolicy, LPCTSTR name, CBASIS basis, LPCTSTR initCostExpr, LPCTSTR maintenanceCostExpr, LPCTSTR durationExpr, LPCTSTR lookupExpr ) 
   : m_pPolicy( pPolicy )
   , m_name( name )
   , m_gcName( "" )
   , m_basis( basis )
...
//////////////////////////////////////////////////////////////////////
// EnvPolicy Construction/Destruction
//////////////////////////////////////////////////////////////////////

EnvPolicy::EnvPolicy( PolicyManager *pPM )
   : m_pPolicyManager( pPM ),
     m_id( -1 ),
     m_index( -1 ),
     m_name(),
...
EnvPolicy& EnvPolicy::operator = ( EnvPolicy &policy )
   {
...
bool EnvPolicy::ParseOutcomes( EnvPolicy *pPolicy, CString &outcomeStr, MultiOutcomeArray &moa )
   {
...
//void EnvPolicy::FixUpOutcomePolicyPtrs()
//   {
...
bool EnvPolicy::SetOutcomes( LPCTSTR outcomes )
   {
...
float EnvPolicy::GetMaxApplicationArea( int idu )
   {
...
/*
void EnvPolicy::SetParserVars( MapLayer *pLayer, int record )
   {
...
/*
float EnvPolicy::EvaluatePreference()
   {
   // return -3 to +3
...
ObjectiveScores *EnvPolicy::FindObjective( LPCTSTR objName )
   {
...
float EnvPolicy::GetGoalScore( int objectiveIndex )  // NOTE:  -1=global objective
   {
...
void EnvPolicy::GetIduObjectiveScores( int idu, float* scores )
   {
...
float EnvPolicy::GetIduObjectiveScore( int idu, int objective )  // -1 for global, zero-based for rest
   {
   ...
   // NOTE:  Assumes EnvPolicy::SetParserVars() has already been called for this idu!!!
...
//void EnvPolicy::NormalizeOutcomeProbabilities()
//   {
...
PolicyArray& PolicyArray::operator = ( PolicyArray &policyArray )
   ...
   for ( int i=0; i < policyArray.GetSize(); i++ )
      Add( new EnvPolicy( *policyArray.GetAt( i ) ) );
...
void PolicyArray::RemoveAll()
   ...
   CArray< EnvPolicy*, EnvPolicy*>::RemoveAll(); 
   }     
...
PolicyManager::~PolicyManager()
   ...
   /*
   if ( EnvPolicy::m_parserVars != NULL )
      {
      delete EnvPolicy::m_parserVars;
      EnvPolicy::m_parserVars = NULL;
      } */
   }


EnvPolicy *PolicyManager::FindPolicy( LPCTSTR name, int *index )
   {
   int len = lstrlen( name );

   for ( int i=0; i < GetPolicyCount(); i++ )
      {
      EnvPolicy *pPolicy = GetPolicy( i );
   ...
   for ( int i=0; i < GetDeletedPolicyCount(); i++ )
      {
      EnvPolicy *pPolicy = GetDeletedPolicy( i );
      if ( ( lstrcmp( name, pPolicy->m_name ) == 0 ) && ( pPolicy->m_name.GetLength() == len )  )
         {
         if ( index != NULL )
            *index = i;
...
EnvPolicy *PolicyManager::GetPolicyFromID( int id )
   {
...
EnvPolicy *PolicyManager::GetDeletedPolicyFromID( int id )
   {
...
void PolicyManager::DeletePolicy( int index )
   {
   EnvPolicy *pPolicy = m_policyArray[ index ];
...
int PolicyManager::CompilePolicies( MapLayer *pLayer )
   ...
   for ( int i=0; i < GetPolicyCount(); i++ )
      ...
      EnvPolicy *pPolicy = (EnvPolicy*) GetPolicy( i );
...
bool PolicyManager::CompilePolicy( EnvPolicy *pPolicy, MapLayer *pLayer )
   {
   // compile site query
   ...
   if ( pPolicy->m_prefSelStr.IsEmpty() )
      ...
      if ( EnvPolicy::m_parserVars == NULL )
         EnvPolicy::m_parserVars = new double[ m_pEnvModel->m_pIDULayer->GetFieldCount() ];
      ...
      for ( int i=0; i < varCount; i++ )
         ...
         int index = col;
         if ( EnvPolicy::m_usedParserVars.Lookup( col, index ) )
            {
            ASSERT( col == index );
            }
         else  // add it
            EnvPolicy::m_usedParserVars.SetAt( col, col );

         pPolicy->m_pParser->redefineVar( varName, EnvPolicy::m_parserVars+index); 
...
bool PolicyManager::ValidateOutcomeFields( EnvPolicy *pPolicy, MapLayer *pLayer )
   {
   poPolicyName = pPolicy->m_name;
...
void PolicyManager::BuildPolicySchedule()
   ...
   for ( int i=0; i < GetPolicyCount(); i++ )
      ...
      if (pPolicy->m_use && pPolicy->m_isScheduled)
         {
         POLICY_SCHEDULE ps(pPolicy->m_startDate, pPolicy->m_endDate, pPolicy);
...
///////////////////////////////////////////////////////////////////////////////
// Functions
OutcomeFunction* OutcomeFunction::MakeFunction( LPCTSTR functionName, EnvPolicy *pPolicy )
   {
...
void EnvPolicy::GetSiteAttrFriendly( CString &str, int options )
   {
...
void EnvPolicy::GetOutcomesFriendly( CString &str )
   {
   str.Empty();
...
void EnvPolicy::GetScoresFriendly( CString &str )
   {
   str.Empty();
...
int PolicyManager::AddPolicy( EnvPolicy *pPolicy )
   {
   ...
   while( pXmlPolicy != NULL )
      ...
      if ( ok )
         ...
         if ( pXmlOutcome != NULL )
            ...
            if ( pXmlOutcomes != NULL )
               ...
               while( pXmlOutcome != NULL )
                  ...
                  EnvPolicy::ParseOutcomes( &policy, outcomeStr, moa );
         ...
         if ( ! deleted  ) // && ( (policy.m_isShared && gpDoc->m_loadSharedPolicies) || ( policy.m_originator.CompareNoCase( gpDoc->m_userName ) == 0 ) ) )
            ...
            EnvPolicy *pPolicy = new EnvPolicy( policy );
...
PolicyConstraint *PolicyManager::LoadXmlConstraint( TiXmlElement *pXmlConstraint, EnvPolicy *pPolicy )
   {
