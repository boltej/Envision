...
*/
// EnvPolicy.h: interface for the EnvPolicy class.
//
//////////////////////////////////////////////////////////////////////
...
class OutcomeFunction
   {
   public:
      OutcomeFunction( EnvPolicy *pPolicy ) : m_pPolicy( pPolicy ){}
   ...
   public:
      ...
      static OutcomeFunction* MakeFunction( LPCTSTR functionName, EnvPolicy *pPolicy );
   };
...
class ENVAPI OutcomeInfo
   ...
   public:
      OutcomeInfo( EnvPolicy *_pPolicy, LPCTSTR _field, VData _value, int _delay, OutcomeDirective *pDirective=NULL );
//         : value( _value ), col( -1 ), m_pFunction( NULL ), m_pDirective( pDirective )
//         { lstrcpy( field, _field ); }

      OutcomeInfo(EnvPolicy *_pPolicy, OutcomeFunction *pFunction )
         : pPolicy(_pPolicy), col( -1 ), m_pFunction( pFunction ), m_pDirective( NULL ), m_pMapExpr( NULL )
         { field[ 0 ]=NULL; value.type = TYPE_NULL; }

      OutcomeInfo( EnvPolicy *_pPolicy ) : pPolicy(_pPolicy), col( -1 ), m_pFunction( NULL ), m_pDirective( NULL ), m_pMapExpr( NULL ) { field[ 0 ]=NULL; value.type = TYPE_NULL; }
...
class ENVAPI BufferOutcomeFunction : public OutcomeFunction
   ...
   public:
      BufferOutcomeFunction( EnvPolicy *pPolicy ) : OutcomeFunction( pPolicy ), m_pArgs( NULL ) {}
...
class ENVAPI AddPointOutcomeFunction : public OutcomeFunction
   ...
   public:
      AddPointOutcomeFunction( EnvPolicy *pPolicy ) : OutcomeFunction( pPolicy ), m_pArgs( NULL ), m_x( 0 ), m_y( 0 ) {}
...
class IncrementOutcomeFunction : public OutcomeFunction
   ...
   //
   public:
      IncrementOutcomeFunction( EnvPolicy *pPolicy ) : OutcomeFunction( pPolicy ), m_pArgs( NULL ), m_pLayer( NULL ), m_col( -1 ), m_increment( 0 ) {}
...
class ExpandOutcomeFunction : public OutcomeFunction
   ...
   public:
      ExpandOutcomeFunction( EnvPolicy *pPolicy ) : OutcomeFunction( pPolicy ), m_pArgs( NULL ), m_pQuery( NULL ), m_colArea( -1 ), m_area( 0 ), m_requireAdjacent( true ) {}
...
{
friend class EnvPolicy;

private:
   PolicyConstraint() {}  // prevent default constructor

public:
   PolicyConstraint( EnvPolicy *pPolicy, LPCTSTR name, CBASIS basis, LPCTSTR initCostExpr, LPCTSTR maintenanceCostExpr, LPCTSTR durationExpr, LPCTSTR lookup );
...
public:
	EnvPolicy( PolicyManager *pPM );
   EnvPolicy( EnvPolicy &policy ) { *this = policy; }      // copy constructor
	~EnvPolicy();              // CFM: For some reason, having a virtual destructor causes problems when
                           //      a dll "news" a EnvPolicy, which is needed for the PolicyMetaProcess
   EnvPolicy& operator = ( EnvPolicy& );
...
public:
   static bool ParseOutcomes( EnvPolicy *pPolicy, CString &outcomeStr, MultiOutcomeArray & );  // array version
...
class ENVAPI PolicyArray : public CArray< EnvPolicy*, EnvPolicy* >
{
public:
...
public:
   ...
   int     AddPolicy( EnvPolicy *pPolicy );  // returns index of added item
   ...
   EnvPolicy *FindPolicy( LPCTSTR name, int *index=NULL );
   ...
   EnvPolicy *GetPolicy( int i ) { return m_policyArray[ i ]; }
   EnvPolicy *GetPolicyFromID( int id );
   ...
   EnvPolicy *GetDeletedPolicy( int i ) { return m_deletedPolicyArray[ i ]; }
   EnvPolicy *GetDeletedPolicyFromID( int id );
   void    RemoveAllDeletedPolicies( void ){ m_deletedPolicyArray.RemoveAll(); }

   int     CompilePolicies( MapLayer *pLayer );
   bool    CompilePolicy( EnvPolicy *pPolicy, MapLayer *pLayer );
   bool    ValidateOutcomeFields( EnvPolicy *pPolicy, MapLayer *pLayer );
...
protected:
   PolicyConstraint *LoadXmlConstraint( TiXmlElement *pXmlConstraint, EnvPolicy *pPolicy );
