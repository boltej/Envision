...
BOOL Reporter::Init( EnvContext *pEnvContext, LPCTSTR initStr ) 
   ...
   for ( int j=0; j < (int) m_outputArray.GetSize(); j++ )
      ...
      if ( pEngine == NULL )
         m_mapExprEngineArray.Add( pEngine = new MapExprEngine( pOutput->m_pMapLayer ) );

      pOutput->m_pMapExprEngine = pEngine;

      // create a MapExpr for this output
      pOutput->m_pMapExpr = pEngine->AddExpr( pOutput->m_name, pOutput->m_expression, pOutput->m_query );
      ...
      if ( pOutput->m_use && pOutput->m_stratifyByStr.GetLength() > 0
            && pOutput->m_pMapLayer->GetFieldCol( pOutput->m_stratifyByStr ) )
         {
         ...
         if ( fields >= 0 )
            ...
            CString name;
            if ( pOutput->m_pGroup != NULL )
               name.Format( "%s-%s_by_%s", (LPCTSTR) pOutput->m_pGroup->m_name, (LPCTSTR) pOutput->m_name, (LPCTSTR) pOutput->m_stratifyField );
            else            
      ...
      if ( pOutput->m_pGroup == NULL && pOutput->m_makePivotTable )
...
bool Reporter::LoadXml( EnvContext *pEnvContext, LPCTSTR filename )
   ...
   XML_ATTR rootAttrs[] = {
         ...
         //{ "layer",     TYPE_STRING,  &layer,           false, 0 }, deprecated
   ...
   //   {
   //   if ( isdigit( layer[ 0 ] ) )
   //      m_pMapLayer = pEnvContext->pMapLayer->m_pMap->GetLayer( atoi( layer ) );
   //   else
   //      m_pMapLayer = pEnvContext->pMapLayer->m_pMap->GetLayer( layer );
   //
   //   if ( m_pMapLayer == NULL )
   //      {
   //      CString msg;
   //      msg.Format( "Error finding layer [%s] specified in Reporter input file '%s'", layer, filename );
   //      Report::ErrorMsg( msg );
   //      return false;
   //      }         
   //   }
   ...
   ///////this->m_pMapExprEngine = new MapExprEngine( m_pMapLayer );
   ///////m_colArea = m_pMapLayer->GetFieldCol( "AREA" );
   ...
   if ( LoadXmlOutputs( pXmlRoot, NULL, pIDULayer, filename ) > 0 )
      {
      CString msg;
   ...
   while( pXmlOutputGroup != NULL )
      ...
      ok = TiXmlGetAttributes( pXmlOutputGroup, groupAttrs, filename );
      if ( ! ok )
         {
         delete pGroup;
         continue;
      ...
      if ( pGroup->m_pMapLayer == NULL )
         {
         CString msg;
         msg.Format( "Reporter: Unrecognized 'layer' attribute for output group '%s'- must be 'sum', 'pctArea' or 'areaWtMean'.  This output group will be ignored...", (LPCTSTR) pGroup->m_name );
         Report::ErrorMsg( msg );
         pGroup->m_use = false;
         }
 
      //////////////if ( stratifyBy != NULL )
      //////////////   pGroup->ParseFieldSpec( stratifyBy, m_pMapLayer );

      m_outputGroupArray.Add( pGroup );

      // get outputs
      LoadXmlOutputs( pXmlOutputGroup, pGroup, pIDULayer, filename );

      // following moved to init()
      /////////////// set up output data object for this group
      /////////////ASSERT( pGroup->m_pData == NULL );
      /////////////int cols = 1;  // time
      /////////////int varCount = 0;  // variables in use
...
int Reporter::LoadXmlOutputs( TiXmlElement *pXmlParent, OutputGroup *pGroup,  MapLayer *pIDULayer, LPCTSTR filename )
   {
   ...
   while( pXmlOutput != NULL )
      ...
      if ( ok )
         ...
         if ( pOutput->m_pMapLayer == NULL )
            {
            CString msg;
            msg.Format( "Reporter: Unrecognized 'layer' attribute for output '%s'- must be 'sum', 'pctArea' or 'areaWtMean'.  This output will be ignored...", (LPCTSTR) pOutput->m_name );
            Report::ErrorMsg( msg );
            pOutput->m_use = false;
            }
