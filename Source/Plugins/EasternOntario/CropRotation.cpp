...
CropRotationProcess::CropRotationProcess( void )
   : m_id( NITROGEN )   
   , m_colLulc( -1 )
   , m_colRotation( -1 )
   , m_colArea( -1 )
...
bool CropRotationProcess::Init( EnvContext *pEnvContext, LPCTSTR initStr )
   {
   // load table
   ...
   theProcess->m_outVarCountCropRotation = 0;
   theProcess->m_outVarIndexCropRotation = 0;
   ...
   // get a list of all LULC vaollues included in a rotation and do those as well
   for ( int i=0; i < (int) this->m_rotationArray.GetSize(); i++ )
      {
      Rotation *pRotation = m_rotationArray.GetAt( i );
...
bool CropRotationProcess::InitRun( EnvContext *pContext )
   {
   // also initializates individual crop area output vars
   AllocateCropRotations( (MapLayer*) pContext->pMapLayer );

   return true;
   }


bool CropRotationProcess::Run( EnvContext *pContext, bool useAddDelta )
   {
...
void CropRotationProcess::AllocateCropRotations( MapLayer *pLayer )
   {
   ...
   // has the largest allocation deficit, expressed as a percent ot the target allocation
   for ( int i=0; i < iduCount; i++ )
      {
      int idu = iduArray[ i ];
...
bool CropRotationProcess::LoadXml( EnvContext *pContext, LPCTSTR filename )
   {
