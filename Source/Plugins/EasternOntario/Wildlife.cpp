...
bool WildlifeModel::Init( EnvContext *pEnvContext, LPCTSTR initStr )
   ...
   for ( int i=0; i < (int) m_groupArray.GetSize(); i++ )
      ...
      label += pGroup->m_name;
      if ( i == 0 )
         theModel->m_outVarIndexWildlife = theModel->AddOutputVar( label, pGroup->m_habCapacity, label );
      else
         theModel->AddOutputVar( label, pGroup->m_habCapacity, label );
      ...
      ok = theModel->CheckCol( pEnvContext->pMapLayer, pGroup->m_col, field, TYPE_FLOAT, CC_AUTOADD );
      if ( ! ok ) 
         pGroup->m_col = -1;
      }
      
   theModel->m_outVarCountWildlife = (int) m_groupArray.GetSize();
...
bool WildlifeModel::Run( EnvContext *pContext, bool useAddDelta )
   ...
   for ( int j=0; j < (int) m_groupArray.GetSize(); j++ )
      ...
      pGroup->m_habCapacity /= totalArea;   // this makes this an area weighted average across SLCs
...
void WildlifeModel::GetSLCScores( SLC *pSLC, MapLayer *pLayer, int year )
   ...
   for ( int i=0; i < iduCount; i++ )
      ...
      float area = 0;
      pLayer->GetData( idu, theModel->m_colArea, area );
   ...
   for ( int i=0; i < (int) m_groupArray.GetSize(); i++ ) 
      ...
      if ( pGroup->m_slcUsedSppCount > 0 )
         pGroup->m_slcHabCapacity /= pGroup->m_slcUsedSppCount;   // this is the average score across spp
      }
   }
...
bool WildlifeModel::LoadHabitatTable( LPCTSTR habScoreTable, LPCTSTR lulcField, MapLayer *pIduLayer )
   ...
   bool ok = theModel->CheckCol( (const MapLayer*) pIduLayer, this->m_colLulc, lulcField, TYPE_LONG, CC_MUST_EXIST );
