...
NAESI::NAESI( void )
   ...
   , m_wetlandCov( NULL ) 
   {    
   theModel->m_outVarCountNaesi = 5;
   theModel->m_outVarIndexNaesi = theModel->AddOutputVar( "Forest Cover", m_forestCover, "" );
   theModel->AddOutputVar( "Wetland Cover", m_wetlandCover, "" );  
   theModel->AddOutputVar( "Wetland In Landscape", m_wetlandInLandscape, "" );//number of wetlands with wetlands within 100 m (polygon based)
   theModel->AddOutputVar( "Grassland Cover", m_grassCover, "" );
   theModel->AddOutputVar( "Farmland Ecosys Serv", m_farmEcoServ, "" );
   theModel->AddOutputVar( "Forest Patches < 200ha", m_forestPatchCount, "" );
   }
...
bool NAESI::Init( EnvContext *pEnvContext, LPCTSTR initStr )
   {   
   ...
   theModel->CheckCol( pLayer, m_colLulcA, _T("LULC_A"), TYPE_LONG, CC_MUST_EXIST );//
   theModel->CheckCol( pLayer, m_colForestCover, _T("ForestCov"), TYPE_FLOAT, CC_AUTOADD );//
   theModel->CheckCol( pLayer, m_colWetlandCover, _T("WetCov"), TYPE_FLOAT, CC_AUTOADD );//
   theModel->CheckCol( pLayer, m_colNearByWetland, _T("WetNear"), TYPE_LONG, CC_AUTOADD );//
   theModel->CheckCol( pLayer, m_colFarmlandGrasslandHabitatArea, _T("GrassCov"), TYPE_FLOAT, CC_AUTOADD );//
   theModel->CheckCol( pLayer, m_colFarmlandEcosystemServices, _T("FarmEcoS"), TYPE_FLOAT, CC_AUTOADD );//
   Run( pEnvContext, false );
   return true;
   }


bool NAESI::InitRun( EnvContext *pContext )
   {
   return true;
   }


bool NAESI::Run( EnvContext *pContext, bool useAddDelta )
   {
   ...
   for ( int i=0; i < slcCount; i++ )
      ...
      if ( pSLC->m_slcNumber >= 0 )
         ...
         for ( int j=0; j < (int) pSLC->m_iduArray.GetSize(); j++ )
            ...
            theModel->UpdateIDU( pContext, idu, m_colForestCover, m_forestCov[i], useAddDelta );  
            theModel->UpdateIDU( pContext, idu, m_colWetlandCover, m_wetlandCov[i], useAddDelta );  
            theModel->UpdateIDU( pContext, idu, m_colFarmlandGrasslandHabitatArea, m_grassCov[i], useAddDelta );  
            }
...
float NAESI::GetCover(SLC *pSLC, MapLayer *pLayer,int value )
   {
   float slcArea = 0;
   float totalScore = 0;
   float numerator=0;
...
int NAESI::GetWetlandInLandscape(SLC *pSLC, MapLayer *pLayer, EnvContext *pEnvContext, bool useAddDelta )
   {
   float slcArea = 0.0f;
   ...
   for ( int i=0; i < (int) pSLC->m_iduArray.GetSize(); i++ )
      ...
      if ( lulc == 4 ) //wetland
         ...
         theModel->UpdateIDU( pEnvContext, idu,m_colNearByWetland,nearbyWetlands, useAddDelta ); //reported for each IDU
         if (nearbyWetlands > 0)
            total++;
         }
...
float NAESI::GetFarmlandEcosytemServices(SLC *pSLC, MapLayer *pLayer, EnvContext *pEnvContext, bool useAddDelta )
   {
   float slcArea = 0.0f;  
   float totalScore = 0;
   float numerator=0;
   ...
   for ( int i=0; i < (int) pSLC->m_iduArray.GetSize(); i++ )
      ...
      if ( lulc == 2 ) //Agriculture...look at neighbors within 3 km, sum the natural area within that distance
         ...
         theModel->UpdateIDU( pEnvContext, idu,m_colFarmlandEcosystemServices,nearbyNaturalArea/nearbyTotalArea, useAddDelta ); //reported for each IDU
         }
      }   
...
int NAESI::GetForestPatchCount( SLC *pSLC, MapLayer *pLayer )
   {
