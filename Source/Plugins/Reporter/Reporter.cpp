   ...
   for (int k = 0; k < (int)outputArray.GetSize(); k++)
      ...
      if (pOutput->m_use)
         ...
         if (pOutput->m_varType == VT_MIN)
            pOutput->m_value = LONG_MAX;
         else if (pOutput->m_varType == VT_MAX)
            pOutput->m_value = LONG_MIN;
      ...
      if (pOutput->m_use && pOutput->m_pMapLayer == pLayer)
         ...
         if (ok)  // passed query?
            ...
            switch (pOutput->m_varType)
               ...
               case VT_MIN:
               {
               double value = pOutput->m_pMapExpr->GetValue();
               if (value < pOutput->m_value)
               ...
               if (attrIndex >= 0)
                  ...
                  if (found && index >= 0)
                     ...
                     switch (pOutput->m_varType)
                        ...
                        {
                        double value = pOutput->m_pMapExpr->GetValue();
                        if (value < pOutput->m_stratifiedValues[index])
                           pOutput->m_stratifiedValues[index] = value;
                        }
                        break;
   ...
   while (pXmlOutput != NULL)
      ...
      if (ok)
         ...
         switch (type[0])
            ...
            case 'M':
            {
            if (type[1] == 'e' || type[1] == 'E') pOutput->m_varType = VT_MEAN;
            else if (type[1] == 'i' || type[1] == 'I') pOutput->m_varType = VT_MIN;
            else if (type[1] == 'a' || type[1] == 'A') pOutput->m_varType = VT_MAX;
            }
            break;          // "mean"
