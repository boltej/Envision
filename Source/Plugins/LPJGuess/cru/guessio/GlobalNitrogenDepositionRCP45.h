...
//   #include <stdlib.h>
//   #include <string.h>
//   #include "E:\Libs\Large_files_(no_bu)\ndep_cc\binfiles\GlobalNitrogenDepositionRCP45.h"
//
...
const int GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH=1617;
...
public:
	...
	bool getnext(GlobalNitrogenDepositionRCP45& obj) {
		...
		for (i=131;i>=0;i--) obj.NOyWet[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,23,1e-005,0);
		for (i=131;i>=0;i--) obj.NOyDry[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,23,1e-005,0);
		for (i=131;i>=0;i--) obj.NHxWet[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,27,1e-005,0);
		for (i=131;i>=0;i--) obj.NHxDry[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,25,1e-005,0);
	...
	bool getindex(GlobalNitrogenDepositionRCP45& obj) {
		...
		long begin = 0;
		long end = GLOBALNITROGENDEPOSITIONRCP45_NRECORD;
		...
		while (begin < end) {
			...
			if (c < 0) {
				...
				for (int i=131;i>=0;i--) obj.NOyWet[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,23,1e-005,0);
				for (int i=131;i>=0;i--) obj.NOyDry[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,23,1e-005,0);
				for (int i=131;i>=0;i--) obj.NHxWet[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,27,1e-005,0);
				for (int i=131;i>=0;i--) obj.NHxDry[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,25,1e-005,0);
