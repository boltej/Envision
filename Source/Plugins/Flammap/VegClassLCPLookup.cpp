...
VegClassLCPLookup::VegClassLCPLookup(const char* pcFName, const int canopyCoverUnits, const int canopyHeightUnits, const int canopyBaseHeightUnits, const int canopyBulkDensityUnits)
   ...
   for (int i = 0; i < recordsTtl; i++)
      ...
      if (!inData.Get(vegClassCol, i, VegClass))
         {
         gpFlamMapAP->m_runStatus = -3; return;
         }
      if (!inData.Get(variantCol, i, Variant)) { gpFlamMapAP->m_runStatus = -4; return; }
      if (!inData.Get(fuelModelCol, i, FuelModel)) { gpFlamMapAP->m_runStatus = -5; return; }
      if (fmodelPlusCol < 0)
         FModelPlus = -1;
      else
         if (!inData.Get(fmodelPlusCol, i, FModelPlus)) { gpFlamMapAP->m_runStatus = -6; return; }

      if (regionCol < 0)
         region = 0;
      else
         if (!inData.Get(regionCol, i, region)) { gpFlamMapAP->m_runStatus = -7; return; }

      if (pvtCol < 0)
         pvt = 0;

      else
         if (!inData.Get(pvtCol, i, pvt)) { gpFlamMapAP->m_runStatus = -8; return; }

      if (!inData.Get(canopyCoverCol, i, CanopyCover)) { gpFlamMapAP->m_runStatus = -9; return; }
      if (!inData.Get(standHeightCol, i, StandHeight)) { gpFlamMapAP->m_runStatus = -10; return; }
      if (!inData.Get(baseHeightCol, i, BaseHeight)) { gpFlamMapAP->m_runStatus = -11; return; }
      if (!inData.Get(bulkDensityCol, i, BulkDensity)) { gpFlamMapAP->m_runStatus = -12; return; }
      ...
      switch (canopyCoverUnits)
         ...
         switch (CanopyCover)
            {
            case 1: tCover = 10; break;
            case 2: tCover = 30; break;
            case 3: tCover = 60; break;
            case 4: tCover = 75; break;
            default: tCover = 0;
            }
         CanopyCover = tCover;
         break;
         ...
         for (int _variant : vars)
            ...
            for (int _region : rgns)
               ...
               if ((pvt == _pvt || pvt == -99)
                  ...
                  {
                  AddValues(VegClass, _region, _pvt, _variant, (short)FuelModel,
                     (short)CanopyCover,
                     (short)StandHeight,
                     (short)BaseHeight,
                     (short)(BulkDensity),
