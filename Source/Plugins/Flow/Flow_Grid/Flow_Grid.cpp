...
float Flow_Grid::Init_Year(FlowContext *pFlowContext)
   ...
   for (MapLayer::Iterator idu = pEnvContext->pMapLayer->Begin(); idu != pEnvContext->pMapLayer->End(); idu++)
      ...
      if (harvestOffset>0 && harvestOffset <50) //if the value is outside this range, that area is not part of the overall timber strategy (it is agricultural or developed)
         ...
         pFlowContext->pFlowModel->UpdateIDU(pFlowContext->pEnvContext, idu, pFlowContext->pFlowModel->m_colLai, lai, 1);
         pFlowContext->pFlowModel->UpdateIDU(pFlowContext->pEnvContext, idu, m_colRotationAge, rotationAge, 1);
         pFlowContext->pFlowModel->UpdateIDU(pFlowContext->pEnvContext, idu, m_colHeight, height, 1);
...
float Flow_Grid::Init_Small_Watershed_Year(FlowContext *pFlowContext)
   {
   //10YrsAG 10YrsPINE 25YrsPINE 35Yrs 35YrAG
   int hruCount = pFlowContext->pFlowModel->GetHRUCount();
   EnvContext *pEnvContext = pFlowContext->pEnvContext;
   ...
   for (MapLayer::Iterator idu = pEnvContext->pMapLayer->Begin(); idu != pEnvContext->pMapLayer->End(); idu++)
      ...
      int age = -1;
      pLayer->GetData(idu, m_colRotationAge, age);
      pFlowContext->pFlowModel->UpdateIDU(pFlowContext->pEnvContext, idu, m_colRotationAge, age, 1);
      pLayer->SetData(idu, m_colRotationAge, age+1);
      }
   pLayer->m_readOnly = true;
   return 1.0f;
   }

float Flow_Grid::Init_FIRE_Grid_Fluxes( FlowContext *pFlowContext )
   {
   ...
   EnvExtension::CheckCol(pFlowContext->pEnvContext->pMapLayer, m_colHarvest, _T("Harvest"), TYPE_INT, CC_AUTOADD);
   pFlowContext->pFlowModel->AddInputVar(_T("Harvest Scenario"), m_harvestFilenameOffSet, _T("0=first, 1, 2, 3, 4"));
...
float Flow_Grid::Init_Grid_Fluxes(FlowContext *pFlowContext)
{
	...
	m_col_lamdaGW = pGWParam_Table->GetFieldCol("lamda");
	m_col_wpGW = pGWParam_Table->GetFieldCol("wp");
...
#pragma omp parallel for 
   ...
   float Flow_Grid::UpdateHBVTreeGrowth(FlowContext *pFlowContext)
   {
	   if (pFlowContext->timing & 1) // Init()
		   return Flow_Grid::Init_FIRE_Grid_Fluxes(pFlowContext);
	   ...
	   int catchmentCount = pFlowContext->pFlowModel->GetCatchmentCount();
	   int hruCount = pFlowContext->pFlowModel->GetHRUCount();
	   int currYear = pFlowContext->pFlowModel->GetCurrentYear();
	   int dayOfSimulation = (int)time;

	   hruCount = pFlowContext->pFlowModel->GetHRUCount();
	   EnvContext *pEnvContext = pFlowContext->pEnvContext;
	   MapLayer *pLayer = (MapLayer*)pEnvContext->pMapLayer;
	   pLayer->m_readOnly = false;
	   ...
	   int colFireYear = -1;
	   int colDisturb = -1;
	   EnvExtension::CheckCol(pFlowContext->pEnvContext->pMapLayer, colFireYear, _T("FIRE_YEAR"), TYPE_INT, CC_AUTOADD);
	   EnvExtension::CheckCol(pFlowContext->pEnvContext->pMapLayer, colDisturb, _T("Disturb"), TYPE_INT, CC_AUTOADD);
	   for (MapLayer::Iterator idu = pEnvContext->pMapLayer->Begin(); idu != pEnvContext->pMapLayer->End(); idu++)
	   {
		   //Get the harvest unit from map
		   int lulcA = -1;
		   ...
		   pLayer->GetData(idu, colFireYear, fireYear);
		   int harvest = 0;
		   pLayer->GetData(idu, m_colHarvest, harvest);  // get HRU info
		   if (fireYear == currYear ) //if the polygon burned
		   {
			   pLayer->SetData(idu, m_colRotationAge, 0);
			   pLayer->SetData(idu, colDisturb, 1);
		   }
		   //Get the lai for that harvest unit from the lai table
		   if (lulcA == 2) //if the polygon is forested
		   {
			   ...
			   float height = m_pAgeHeight_Table->IGet(rotationAge);
			   //Set LAI in the IDU layer
			   pFlowContext->pFlowModel->UpdateIDU(pFlowContext->pEnvContext, idu, pFlowContext->pFlowModel->m_colLai, lai, ADD_DELTA);
			   //pFlowContext->pFlowProcess->UpdateIDU(pFlowContext->pEnvContext, idu, m_colRotationAge, rotationAge, 1);
			   pFlowContext->pFlowModel->UpdateIDU(pFlowContext->pEnvContext, idu, m_colHeight, height, ADD_DELTA);
			   //	pLayer->SetData(idu, pFlowContext->pFlowModel->m_colLai, lai);
			   //pLayer->SetData(idu, m_colRotationAge, rotationAge);
			   //	pLayer->SetData(idu, m_colHeight, height);
		   }
...
float Flow_Grid::UpdateTreeGrowth(FlowContext *pFlowContext)
   ...
   int dayOfSimulation = (int)time;
	...
	for (MapLayer::Iterator idu = pEnvContext->pMapLayer->Begin(); idu != pEnvContext->pMapLayer->End(); idu++)
      ...
      int harvest = 0;
      pLayer->GetData(idu, m_colHarvest, harvest);  // get HRU info
      if (dayOfSimulation == 881 && harvest == 1) //if the polygon is to be harvested
         {
         pLayer->SetData(idu, m_colRotationAge, 0);
         }
		//Get the lai for that harvest unit from the lai table
...
float Flow_Grid::UpdateSmallWatershedTreeGrowth(FlowContext *pFlowContext)
{

   if (pFlowContext->timing & 2) // Init()
      return Flow_Grid::InitRun_Grid_Fluxes(pFlowContext);
   if (pFlowContext->timing & 4) // Init()
      return Flow_Grid::Init_Small_Watershed_Year(pFlowContext);
...
//   EnvContext *pEnvContext = pFlowContext->pEnvContext;
 //  MapLayer *pLayer = (MapLayer*)pEnvContext->pMapLayer;
   ...
   for (MapLayer::Iterator idu = pLayer->Begin(); idu != pLayer->End(); idu++)
   {
      ...
      int harvest = 0;
      pLayer->GetData(idu, m_colHarvest, harvest);  // get HRU info
      if (dayOfSimulation == 900 && harvest == 1) //if the polygon is to be harvested
         {
         pLayer->SetData(idu, m_colRotationAge, 0);//Harvest this unit
         }
      //Get the lai for that harvest unit from the lai table
      if (lulcA == 2) //if the polygon is forested
         {
         pLayer->GetData(idu, m_colRotationAge, rotationAge);
         rotationAge += 1.0f/(365.0f*24.0f);//increment age by 1 hr (or 1/365 of a year)
         float lai = m_pLAI_Table->IGet(rotationAge);
         float height = m_pAgeHeight_Table->IGet(rotationAge);
         //Set LAI in the IDU layer
         pFlowContext->pFlowModel->UpdateIDU(pFlowContext->pEnvContext, idu, pFlowContext->pFlowModel->m_colLai, lai, ADD_DELTA);
         pFlowContext->pFlowModel->UpdateIDU(pFlowContext->pEnvContext, idu, m_colHeight, height, ADD_DELTA);
         pFlowContext->pFlowModel->UpdateIDU(pFlowContext->pEnvContext, idu, m_colRotationAge, rotationAge, ADD_DELTA);
         pLayer->SetData(idu, pFlowContext->pFlowModel->m_colLai, lai);
         pLayer->SetData(idu, m_colRotationAge, rotationAge);
         pLayer->SetData(idu, m_colHeight, height);
         }
      ...
      float Flow_Grid::AdjustForSaturation(FlowContext *pFlowContext)
         {
            ...
            for (int h = 0; h < hruCount; h++)
            {
               ...
               if (m_wDepthGW > totalDepthLower) // if depth of water is greater than groundwater zone depth, move excess to argillic
                  {
                  rechargeAccounting = pSatBelowArgillic->m_volumeWater - (totalDepthLower*pHRU->m_area*phiGw);
				  pPonded->AddFluxFromGlobalHandler(rechargeAccounting / dt, FL_BOTTOM_SOURCE);
                  pSatBelowArgillic->AddFluxFromGlobalHandler(rechargeAccounting / dt, FL_TOP_SINK);
                  m_wDepthGW = totalDepthLower;
                  isSatDeepGround = true;
                  }
               ...
               if (wcUnBelowArgillic > phiGw) //if the lower unsaturated zone is over saturation, move water to argillic
                  {
                  rechargeAccountingArgillic = pUnBelowArgillic->m_volumeWater - (waterDepthLowerUnsat*pHRU->m_area*phiGw);//m3/timestep
				  pPonded->AddFluxFromGlobalHandler(rechargeAccountingArgillic / dt, FL_BOTTOM_SOURCE);//m3/d
                  pUnBelowArgillic->AddFluxFromGlobalHandler(rechargeAccountingArgillic / dt, FL_TOP_SINK);
                  wcUnBelowArgillic = phiGw;
                  isSat5 = true;
                  }
...
// End Unsaturated Below Argillic
 ...
 //                 pArgillic->m_volumeWater -= excess;
  ...
  //                pSatAboveArgillic->m_volumeWater -= exchange;
				  ...
				  pPonded->AddFluxFromGlobalHandler(exchange / dt, FL_BOTTOM_SOURCE);
                  pSatAboveArgillic->AddFluxFromGlobalHandler(exchange / dt, FL_TOP_SINK);
                  isSatShallowGround = true;
               }
               else
   ...
   float Flow_Grid::SRS_6_Layer(FlowContext *pFlowContext)
    ...
    //     return Flow_Grid::AdjustForSaturation_Rates(pFlowContext);
     ...
     //    float time = ttt - startDay;
         ...
         float wpArg = 10.0f; float lamdaArg = 0.1f; float phiArg = 0.3f;
         float HOF_Threshold=25.0f; float wpGW=0.1f; float lamdaGW=3.0f; float ic = 0.01;
         ...
         ok = pGWParam_Table->Lookup(geo, m_col_pleGw, pleGw);        // power law exponent
         ok = pGWParam_Table->Lookup(geo, m_col_phiGw, phiGw);        // power law exponent
         ok = pGWParam_Table->Lookup(geo, m_col_lamdaGW, lamdaGW);        // power law exponent
         ok = pGWParam_Table->Lookup(geo, m_col_wpGW, wpGW);        // power law exponent
...
//         if (pSatBelowArgillic->m_volumeWater / pHRU->m_area*1000.0f > 100.0f) //above 100 mm of water, 
            gwf = CalcSatWaterFluxes(pHRU, pSatBelowArgillic, phiGw, pleGw, kSatGw, satGWToUnsatGW, minElev, pSatBelowArgillic->m_depth, phiGw, pGWParam_Table, pFlowContext);
...
//         if (!isSatDeepGround)
            gwUnsatToSat = CalcBrooksCoreyKTheta(wcUnBelowArgillic, wpArg, phiArg, lamdaArg, kSatArg);
            // End Unsaturated Below Argillic
 ...
 //        if (!isSat5)//if the zone below the argillic is unsaturated, then move water into it
         ...
         if (pSatAboveArgillic->m_volumeWater / pHRU->m_area*1000.0f > 25.0f)
         ...
         if (wcU > wp)
            {
            if (wcU >= phi*0.9f)//greater than field capacity
               { 
               wcU = phi;
               isUnsatShallowGround=true;
               pHRU->m_biomass=2;
            ...
            if (uns < pUnsat->m_volumeWater/pHRU->m_area/dt)//verify to guard against negative water content - don't lose more than 10 percent in any timestep.
               unsatToSat = uns ;
            else
               unsatToSat = pUnsat->m_volumeWater / pHRU->m_area / dt;

            }        

         satToArgillic = pSatAboveArgillic->m_volumeWater*ic;
		 if (satToArgillic < 0.0f)
			 satToArgillic = 0.0f;


         k=0.5f;
         infiltration = pPonded->m_volumeWater / pHRU->m_area * k ;//constant fraction infiltrates  m/d

         if (isUnsatShallowGround )//if saturated
			{
            runoff+=infiltration;
            infiltration=0.0f;
            }
...
ASSERT(infiltration>=0.0f);
         ...
         pSatAboveArgillic->AddFluxFromGlobalHandler(unsatToSat*pHRU->m_area, FL_TOP_SOURCE);
         pSatAboveArgillic->AddFluxFromGlobalHandler(satToArgillic, FL_BOTTOM_SINK);
         pSatAboveArgillic->AddFluxFromGlobalHandler(ssf, FL_USER_DEFINED);
         pSatAboveArgillic->AddFluxFromGlobalHandler(shallowGWOutOfCatchment, FL_BOTTOM_SINK);
                 
         pArgillic->AddFluxFromGlobalHandler(satToArgillic, FL_TOP_SOURCE);
         pArgillic->AddFluxFromGlobalHandler(argillicToUnsat*pHRU->m_area, FL_BOTTOM_SINK);
...
float Flow_Grid::HJA_6_Layer(FlowContext *pFlowContext)
	  {
		  if (pFlowContext->timing & 1) // Init()
			  return Flow_Grid::Init_Grid_Fluxes(pFlowContext);
		  ...
		  AdjustHJAForSaturation(pFlowContext);
...
#pragma omp parallel for
		  for (int h = 0; h < hruCount; h++)
		  {
			  ...
			  //        if (pSatAboveArgillic->m_volumeWater / pHRU->m_area*1000.0f > 25.0f)
			  ssf = CalcSatWaterFluxes(pHRU, pSatAboveArgillic, phi, ple, kSat, satToUnsat, minElev, totalDepthUpper, wcL, pLULC_Table, pFlowContext);
			  float  deepGWFlow = pSatAboveArgillic->m_volumeWater*0.001f;
			  ...
			  if (wcU > wp)
			  {
				  ...
				  uns = CalcBrooksCoreyKTheta(wcU, wp, phi, lamda, kSat);
				  if (uns < pUnsat->m_volumeWater / pHRU->m_area )//verify to guard against negative water content - don't lose more than 10 percent in any timestep.
					  unsatToSat = uns;
				  else
					  unsatToSat = pUnsat->m_volumeWater / pHRU->m_area ;
			  ...
			  pSatAboveArgillic->AddFluxFromGlobalHandler(unsatToSat*pHRU->m_area, FL_TOP_SOURCE);
			  pSatAboveArgillic->AddFluxFromGlobalHandler(ssf, FL_USER_DEFINED);
			  pSatAboveArgillic->AddFluxFromGlobalHandler(deepGWFlow, FL_BOTTOM_SINK);
			  pSatAboveArgillic->AddFluxFromGlobalHandler(shallowGWOutOfCatchment, FL_BOTTOM_SINK);
		  }
	  ...
	  float Flow_Grid::AdjustHJAForSaturation(FlowContext *pFlowContext)
	  {
		  float infiltrationExcess = 0.0f;
		  float soilToStream = 0.0f;
		  float groundwaterToSoil = 0.0f;
...
#pragma omp parallel for
		  for (int h = 0; h < hruCount; h++)
		  {
			  ...
			  float totalDepthUpper =pSatAboveArgillic->m_depth;
			  float wcL = (float)pSatAboveArgillic->m_volumeWater / (pHRU->m_area*totalDepthUpper);
			  ...
			  if (pSatAboveArgillic->m_volumeWater > (totalDepthUpper*pHRU->m_area*phi)) //sat+unsat depth is fully saturated with water
			     {
				  ...
				  pSatAboveArgillic->AddFluxFromGlobalHandler(exchange / dt, FL_TOP_SINK);
				  isSatShallowGround = true;
				  pSatAboveArgillic->m_wc = phi;//water content   
			     }
			  else
				  pSatAboveArgillic->m_wDepth = (float)pSatAboveArgillic->m_volumeWater / pHRU->m_area / phi; //m
																											  //End Saturated Above Argillic
			  ...
			  float waterDepthUpperUnsat = pUnsat->m_depth;/*+ pSatAboveArgillic->m_depth - pSatAboveArgillic->m_wDepth*/;//total depth of the zone above the argillic
			  float m_wDepth = pUnsat->m_volumeWater / pHRU->m_area / phi;
			  wcU = (float)pUnsat->m_volumeWater / (pHRU->m_area*waterDepthUpperUnsat);
			  float excess = 0.0f;
			  if (wcU > wp)
			     {
				  if (wcU >= phi)//greater than field capacity
				     {
					  excess = pUnsat->m_volumeWater - (waterDepthUpperUnsat * pHRU->m_area * phi);

					  pUnsat->AddFluxFromGlobalHandler(excess / dt, FL_STREAM_SINK);

					  Reach * pReach = pUnsat->GetReach();
					  if (pReach)
						  if (excess > 0.0f)
							  pReach->AddFluxFromGlobalHandler(excess/dt); //m3/d
			  ...
			  pHRU->m_currentRunoff = excess / dt /pHRU->m_area * 1000.0f; // mm/d
			  pUnsat->m_wDepth = m_wDepth;//m of water without soil.  total depth is waterDepthUpperUnsat!
			  pUnsat->m_wc = wcU;//
								 //End Unsaturated
  ...
  // myElev = pHRU->m_elevation;
   ...
   for (int upCheck = 0; upCheck<pHRU->m_neighborArray.GetSize(); upCheck++)
     ...
     //    otherElev = pUpCheck->m_elevation;
         if (myElev >= otherElev && depth > 0.01f) // calculate my outputs
            {
            ...
            outflow += T*slope*w;
         //   if (pHRUPool->m_volumeWater / pHRU->m_area <= outflow)
         //      outflow = 0.0f;
            pHRUPool->m_waterFluxArray[upCheck + 7] = -T*slope*w;
            }
         if (myElev < otherElev  && depthUp > 0.01f) //calculate my inputs
            {
            ...
            inflow += T*slope*w;
         //   if (pUpCheckLayer->m_volumeWater/pHRU->m_area <= inflow)
          //     inflow = 0.0f;
            pHRUPool->m_waterFluxArray[upCheck + 7] = T*slope*w;
            }
         }
...
float Flow_Grid::CalcSatWaterFluxes(HRU *pHRU, HRUPool *pHRUPool, float phi, float ple, float kSat, float &q1, float minElev)
   ...
   float depth = ((float)pHRUPool->m_volumeWater / pHRU->m_area) / phi;  //actual depth of water filled area
   int pool = pHRUPool->m_pool;
   //HRUPool *pUpperLayer = pHRU->GetPool(pool-1);
   float totalDepth = pHRUPool->m_depth/* + pUpperLayer->m_depth*/; // total depth of the 2 layers
   ...
   for (int upCheck = 0; upCheck<pHRU->m_neighborArray.GetSize(); upCheck++)
      ...
      if (pUpCheck != NULL)
         {
         HRUPool *pUpCheckLayer = pUpCheck->GetPool(pool);
