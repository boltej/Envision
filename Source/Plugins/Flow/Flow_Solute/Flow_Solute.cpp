...
, m_pRateData(NULL)
, m_pDataDeposition(NULL)
, m_pTimeSinceTracerData(NULL)
, m_pTracerData1(NULL)
, m_pTracerData2(NULL)
, m_pTracerData3(NULL)
, m_pTracerData4(NULL)
...
Flow_Solute::~Flow_Solute(void)
   ...
   if (m_pDataDeposition != NULL)
	   delete m_pDataDeposition;

   if (m_pTracerData1 != NULL)
      delete m_pTracerData1;
   if (m_pTracerData2 != NULL)
      delete m_pTracerData2;
   if (m_pTracerData3 != NULL)
      delete m_pTracerData3;
   if (m_pTracerData4 != NULL)
      delete m_pTracerData4;
   if (m_pTracerData5 != NULL)
      delete m_pTracerData5;
   if (m_pTracerData6 != NULL)
      delete m_pTracerData6;
   if (m_pTimeSinceTracerData != NULL)
      delete m_pTimeSinceTracerData;
   }
...
float Flow_Solute::EndRun_Solute_Fluxes(FlowContext *pFlowContext)
   ...
   fileOut.Format("%soutput\\timeSinceTracer.csv", (LPCTSTR)path);
   m_pTimeSinceTracerData->WriteAscii(fileOut);

   fileOut.Format("%soutput\\tracer1.csv", (LPCTSTR)path);
   m_pTracerData1->WriteAscii(fileOut);
   fileOut.Format("%soutput\\tracer2.csv", (LPCTSTR)path);
   m_pTracerData2->WriteAscii(fileOut);
   fileOut.Format("%soutput\\tracer3.csv", (LPCTSTR)path);
   m_pTracerData3->WriteAscii(fileOut);
   fileOut.Format("%soutput\\tracer4.csv", (LPCTSTR)path);
   m_pTracerData4->WriteAscii(fileOut);
   fileOut.Format("%soutput\\tracer5.csv", (LPCTSTR)path);
   m_pTracerData5->WriteAscii(fileOut);
   fileOut.Format("%soutput\\tracer6.csv", (LPCTSTR)path);
   m_pTracerData6->WriteAscii(fileOut);
...
float Flow_Solute::Init_Solute_Fluxes(FlowContext *pFlowContext)
   ...
   EnvExtension::CheckCol(pFlowContext->pFlowModel->m_pCatchmentLayer, m_colField, "HARVEST", TYPE_STRING, CC_AUTOADD);
   ...
   EnvExtension::CheckCol(pFlowContext->pFlowModel->m_pCatchmentLayer, m_colStandAge, "Stand_Age", TYPE_FLOAT, CC_AUTOADD); //get crop number from LULC_B
  ...
  /* m_col_koc = pSV->m_paramTable->GetCol("koc");
   ...
   m_pDataDeposition = new FDataObj();
   m_pDataDeposition->ReadAscii("C:\\Envision\\studyareas\\SRS\\C\\Deposition\\NADP_Deposition.csv",',');

   return 0.0f;
   }

   float Flow_Solute::Init_Run_Solute_Fluxes(FlowContext *pFlowContext)
   {
      int svC = pFlowContext->svCount;
      m_pTracerData1 = new FDataObj(svC + 1, 0);
      m_pTracerData2 = new FDataObj(svC + 1, 0);
      m_pTracerData3 = new FDataObj(svC + 1, 0);
      m_pTracerData4 = new FDataObj(svC + 1, 0);
      m_pTracerData5 = new FDataObj(svC + 1, 0);
      m_pTracerData6 = new FDataObj(svC + 1, 0);
      m_pTimeSinceTracerData = new FDataObj(svC + 1, 0);
...
float Flow_Solute::Solute_Fluxes_SRS_Old(FlowContext *pFlowContext)
{
   ...
   if (pFlowContext->timing & GMT_INITRUN) // Init()
      return Flow_Solute::Init_Run_Solute_Fluxes(pFlowContext);
...
float Flow_Solute::Solute_Fluxes_HJA(FlowContext *pFlowContext)
   {
	if (pFlowContext->timing & GMT_INIT) // Init()
		return Flow_Solute::Init_Solute_Fluxes(pFlowContext);
	...
	float ttt = pFlowContext->time;
	float startDay = pFlowContext->pEnvContext->startYear * 365;
	float time = ttt - startDay;


	int catchmentCount = pFlowContext->pFlowModel->GetCatchmentCount();
	int hruCount = pFlowContext->pFlowModel->GetHRUCount();
	int currYear = pFlowContext->pFlowModel->GetCurrentYearOfRun();
	int dayOfSimulation = (int)time;
	int numDaysInSimulation = (pFlowContext->pEnvContext->endYear - pFlowContext->pEnvContext->startYear) * 365;
	int svC = pFlowContext->svCount;
	float currentTrace0 = (float)dayOfSimulation / (float)numDaysInSimulation*(float)svC;
	int currentTracer = int(currentTrace0);
	int numDaysRepresentedByEachTracer = (float)numDaysInSimulation / (float)svC;
	int currentPeriod = round(time / (float)numDaysInSimulation*(float)svC) + 1;
...
//#pragma omp parallel for
		for (int k = 0; k < svC; k++)   //for each extra state variable
		{
			...
			float appRate = 0.0f;
			if (k == currentTracer)//add this tracer to precipitation
				appRate = pLayer1->m_waterFluxArray[FL_TOP_SOURCE] * 1.0f;//concentration in tracer is 1 kg/m3.  m3/d*1kg/m3=kg/d
			if (appRate > 0.5f)
				int crap = 1;
			MapLayer *pIDULayer = (MapLayer*)pFlowContext->pEnvContext->pMapLayer;
			float e111 = pLayer1->m_waterFluxArray[FL_STREAM_SINK] * cL1; //loss to streams
			Reach * pReach = pLayer1->GetReach();
			float c1 = 0.0f; float d1 = 0.0f;
			float advectiveFluxL1 = 0.0f;
			if (pReach)
			   {
				ReachSubnode *subnode = pReach->GetReachSubnode(0);
				subnode->AddExtraSvFluxFromGlobalHandler(k, -e111); //m3/d   e111 is negative, but we want to add a positive value here (it is a gain)
			...
			// float uptakeEfficiency = 0.5f;
			a1 = pLayer2->m_waterFluxArray[FL_TOP_SINK] * cL2;//upward flux across upper surface of this layer (a loss) -
															  // float appRate = ApplicationRate(time, pLayer2, pFlowContext, k);
			b1 = pLayer2->m_waterFluxArray[FL_TOP_SOURCE] * cL1;//down flux across upper surface of this layer (a gain) precip, but accounted for elsewhere (a flux)
																// b1 += appRate;
			c1 = 0;//upward flux across lower surface of this layer (a gain) - saturated to unsaturated zone
			d1 = pLayer2->m_waterFluxArray[FL_BOTTOM_SINK] * cL2; //down flux across lower surface of this layer (a loss)
																  //float e111 = pLayer2->m_waterFluxArray[FL_STREAM_SINK] * cL2; //loss to streams
			e111 = CalcSatTracerFluxes(pHRU, pLayer2, k);
			...
			if (pHRU->m_lowestElevationHRU && currYear >= pFlowContext->pFlowModel->m_detailedSaveAfterYears && (fmod((double)time, pFlowContext->pFlowModel->m_detailedSaveInterval) < 0.01f))
			{

				float *dayRepresentedByTracer = new float[svC + 1];
				dayRepresentedByTracer[0] = time;
				for (int kk = 1; kk < svC + 1; kk++)
					dayRepresentedByTracer[kk] = (kk*numDaysRepresentedByEachTracer) - (numDaysRepresentedByEachTracer / 2.0f);


				float previousTime = -1;
				if (m_pTimeSinceTracerData->GetRowCount() > 0)
					previousTime = m_pTimeSinceTracerData->Get(0, m_pTimeSinceTracerData->GetRowCount() - 1);
				if (time > previousTime)
				{
					float *data = new float[svC + 1];
					data[0] = (float)time;

					for (int kk = 1; kk < svC + 1; kk++)
					   {
						if (kk == currentPeriod)
							data[kk] = numDaysRepresentedByEachTracer / 2.0f;
						if (kk < currentPeriod)
							data[kk] = (numDaysRepresentedByEachTracer  * (currentPeriod + 1 - kk)) - numDaysRepresentedByEachTracer / 2.0f;
						else
							data[kk] = 0.0f;
					   }
					m_pTimeSinceTracerData->AppendRow(data, svC + 1);
					delete [] dayRepresentedByTracer;

					if (pLayer1->m_volumeWater > 0.00000001f)
					   {
						for (int kk = 1; kk < svC + 1; kk++)
							data[kk] = (float)(pLayer1->GetExtraStateVarValue(kk - 1) / (float)pLayer1->m_volumeWater);
					   }
					m_pTracerData1->AppendRow(data, svC + 1);

					if (pLayer2->m_volumeWater > 0.00000001f)
					   {
						for (int kk = 1; kk < svC + 1; kk++)
							data[kk] = (float)(pLayer2->GetExtraStateVarValue(kk - 1) / (float)pLayer2->m_volumeWater);
					   }
					m_pTracerData2->AppendRow(data, svC + 1);
					delete [] data;
				}
			}
		}
...
#pragma omp parallel for
      for (int k = 0; k<svC; k++)   //for each extra state variable
      {
         ...
         float appRate = 0.0f;
         if (k == currentTracer)//add this tracer to precipitation
            appRate = pLayer6->m_waterFluxArray[FL_TOP_SOURCE]*1.0f;//concentration in tracer is 1 kg/m3.  m3/d*1kg/m3=kg/d
         if (appRate > 0.5f)
            int crap=1;
         MapLayer *pIDULayer = (MapLayer*)pFlowContext->pEnvContext->pMapLayer; 
...
*/
      ...
      //float x = time - (kk+1* numDaysRepresentedByEachTracer) ;
      ...
      if ( pHRU->m_lowestElevationHRU && currYear >= pFlowContext->pFlowModel->m_detailedSaveAfterYears && (fmod((double)time, pFlowContext->pFlowModel->m_detailedSaveInterval ) < 0.01f))
         ...
         if (time > previousTime)
            ...
            if (pLayer3->m_volumeWater > 0.0f)
               {
               for (int kk = 1; kk<svC + 1; kk++)
                  data[kk] = (float)(pLayer3->GetExtraStateVarValue(kk - 1) / (float)pLayer3->m_volumeWater);
               }
            m_pTracerData3->AppendRow(data, svC + 1);

            if (pLayer4->m_volumeWater > 0.000000f)
               {
               for (int kk = 1; kk<svC + 1; kk++)
                  data[kk] = (float)(pLayer4->GetExtraStateVarValue(kk - 1) / (float)pLayer4->m_volumeWater);
               }
            m_pTracerData4->AppendRow(data, svC + 1);

            if (pLayer5->m_volumeWater > 0.000000f)
               {
               for (int kk = 1; kk<svC + 1; kk++)
                  data[kk] = (float)(pLayer5->GetExtraStateVarValue(kk - 1) / (float)pLayer5->m_volumeWater);
               }
            m_pTracerData5->AppendRow(data, svC + 1);

            if (pLayer6->m_volumeWater > 0.000000f)
               {
               for (int kk = 1; kk<svC + 1; kk++)
                  data[kk] = (float)(pLayer6->GetExtraStateVarValue(kk - 1) / (float)pLayer6->m_volumeWater);
               }
            m_pTracerData6->AppendRow(data, svC + 1);
...
float Flow_Solute::Solute_Fluxes_SRS_Managed(FlowContext *pFlowContext)
{
   if (pFlowContext->timing & GMT_INIT) // Init()
      return Flow_Solute::Init_Solute_Fluxes(pFlowContext);
   ...
   float currTime = pFlowContext->time - (int)pFlowContext->time;
   float precip = 0.0f;
   ...
   for (int h = 0; h < hruCount; h++) //for each HRU
   {
      ...
      int hruLayerCount = pHRU->GetPoolCount();
      int harvest = 0;
      pFlowContext->pFlowModel->GetHRUData(pHRU, m_colField, harvest, DAM_FIRST);  // get HRU info
...
#pragma omp parallel for
      for (int k = 0; k<svC; k++)   //for each extra state variable
      {
		  if (k == 0)//nitrate
		      {
			  depConc = m_pDataDeposition->IGet(pFlowContext->time, 3) / 1000.0f;//convert mg/l to kg/m3
			//  deposition = (precip / 1000.0f*pHRU->m_area)*depConc; //m3/d*kg/m3 = kg/d
		     }
		  if (k==1)
		     {
			 // deposition = m_pDataDeposition->IGet(pFlowContext->time, 4);//per mil
		     }
   ...
   //      pFlowContext->pEnvContext->pMapLayer->m_readOnly = false;
         pLayer->m_readOnly = false;
      //   if (dayOfSimulation == 881 && harvest == 1)//May 31 2012 - harvest
      //      pLayer->SetData(pHRU->m_polyIndexArray[0], m_colLAI, 0);

	/*	 if (k == 0)
		    {
			 if ((int)pFlowContext->time == 734858 && harvest == 1) //fert on 4/23/2013 .. assumes start on 1/1/2010
				 appRate = 250.f / LB_PER_KG*pHRU->m_area / M2_PER_HA*ACRE_PER_HA * 0.2f * pFlowContext->timeStep;    //lb/acre *1 kg/m2*m2/m2/km2, and 20% N (this is diamonium phosphate

			 if ((int)pFlowContext->time == 735173 && harvest == 1) //fert on 3/3/2014 .. assumes start on 1/1/2010
				 appRate = 217.f / LB_PER_KG*pHRU->m_area / M2_PER_HA*ACRE_PER_HA * 0.44f * pFlowContext->timeStep;    //lb/acre *1 kg/m2*m2/m2/km2 (44% N - this is Urea
		     }
*/
...
//		 if (/*dayOfSimulation*/pLayer6->m_waterFluxArray[FL_TOP_SOURCE] > 0.0f )
		    deposition = pLayer6->m_waterFluxArray[FL_TOP_SOURCE] * 1.0f; //precip, concentration of 1 kg/m3
         ...
         float advectiveFluxL1 =  c1 + d1 + e111 + deposition ;
         ss = advectiveFluxL1;
         pLayer6->AddExtraSvFluxFromGlobalHandler(k, ss);
         ...
         d1 = pLayer2->m_waterFluxArray[FL_BOTTOM_SINK] * cL2; //down flux across lower surface of this layer (a loss)
                                                               //float e111 = pLayer2->m_waterFluxArray[FL_STREAM_SINK] * cL2; //loss to streams
		// if (pLayer2->m_volumeWater / pHRU->m_area*1000.0f > 25.0f)
            e111 = CalcSatTracerFluxes(pHRU, pLayer2, k);
		// e111 = 0.0f;
      ...
      if (pHRU->m_lowestElevationHRU && (fmod((double)time, pFlowContext->pFlowModel->m_detailedSaveInterval) < 0.01f))
      {
         float *dayRepresentedByTracer = new float[svC + 1];
         dayRepresentedByTracer[0] = time;
         for (int kk = 1; kk<svC + 1; kk++)
   ...
   for (int upCheck = 0; upCheck<pHRU->m_neighborArray.GetSize(); upCheck++)
      ...
      if (pUpCheck != NULL)
         ...
         if (flux<0.0f)//water leaves this grid cell in this direction
            outflow += (flux * conc);//kg/d - this number is negative!
         else
            inflow += (flux * concUp);//kg/d
         }
      }
   float exchange = inflow+outflow;

   return exchange;//outflow has a negative sign, so we add a negative value - effectively subtracting outflow from inflow
   }
/*
