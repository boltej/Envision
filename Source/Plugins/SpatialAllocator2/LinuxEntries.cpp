...
std::shared_ptr<SpatialAllocator> spCleanup(new SpatialAllocator);
SpatialAllocator *theProcess=spCleanup.get();
...
bool APInit( EnvContext *pEnvContext, LPCTSTR initStr )    { return theProcess->Init( pEnvContext, initStr ); }
bool APInitRun( EnvContext *pEnvContext, bool useInitSeed ){ return theProcess->InitRun( pEnvContext, useInitSeed ); }
bool APRun( EnvContext *pEnvContext )                      { return theProcess->Run( pEnvContext );  }
bool APEndRun( EnvContext *pEnvContext )                   { return theProcess->EndRun( pEnvContext );  }
bool APSetup( EnvContext *pContext, HWND hWnd )            { return theProcess->Setup( pContext, hWnd ); }
int  APInputVar( int id, MODEL_VAR** modelVar )            { return theProcess->InputVar( id, modelVar ); }
int  APOutputVar( int id, MODEL_VAR** modelVar )           { return theProcess->OutputVar( id, modelVar ); }
