...
enum TARGET_DOMAIN { TD_UNDEFINED = 0, TD_TOTAL = 2, TD_PERCENT = 3, TD_ATTR = 4 /*unused*/, TD_USEALLOCATION = 5 };
...
enum UPDATE_OPERATOR { UO_REPLACE = 0, UO_LOGICAL_OR };
...
struct ALLOC_SCORE { int record; float score; };  // i == idu for idu-based allocation
...
struct LAYER_INFO
   {
   MapLayer* pMapLayer = nullptr;
   MapExprEngine* pMapExprEngine = nullptr;
   QueryEngine* pQueryEngine = nullptr;

   RandUniform* pRandUnif = nullptr;
   //std::vector<unsigned int> shuffleArray;
   CUIntArray shuffleArray;    // used for shuffling IDUs
   CArray<bool> openRecordsArray;

   ~LAYER_INFO()
      {
      if (pRandUnif != nullptr)
         delete pRandUnif;
      }
   };
...
class TargetContainer
   ...
   public:
      ...
      TARGET_DOMAIN  m_targetDomain;   // what units are being allocated area, pctarea, attr (targetBasis)
      LAYER_INFO* m_pLayerInfo = nullptr;
      CString m_targetValues;          // string containing target source information/values

      CString m_targetBasisField;      // Contains name of field that is the basis for what is used as the target
      int     m_colTargetBasis;        // field being allocated, e.g. the units of the target values,
      // for example, if you want to allocate "Carbon", you would specify a field
      // like "CARBON" as the basis for the allocation.  Default = "AREA"

      CString m_domainField;           // the field used as the basis for "area", "pctArea", taget domains, ignored otherwise
      int     m_colDomainField;        // defaullt is "AREA" for polygon coverages, "Shape_Length" for lines, -1 for points
   ...
   public:
      ...
      TargetContainer()
         ...
         , m_colTargetBasis(-1)
         , m_domainField()
         , m_colDomainField(-1)        // defaullt is "AREA" for polygon coverages, "Shape_Length" for lines, -1 for points
         , m_description()
         , m_status()
         , m_pTargetData(NULL)
         , m_allocationSoFar(0)
      ...
      TargetContainer& operator = (TargetContainer& tc)
         ...
         m_domainField = tc.m_domainField;
         m_colDomainField = tc.m_colDomainField;        // defaullt is "AREA" for polygon coverages, "Shape_Length" for lines, -1 for points
      ...
      void SetTargetParams(LPCTSTR basis, LPCTSTR source, LPCTSTR values, LPCTSTR domain, LPCTSTR domainField, LPCTSTR query);
...
class Allocation : public TargetContainer
   ...
   public:
      ...
      std::vector<ALLOC_SCORE> m_scoreArray;
      ...
      int m_currentScoreIndex;   // this stores the current index of this allocation in the iduScore array
   ...
   public:
      ...
      int   GetCurrentRecord(void) { return m_currentScoreIndex >= m_scoreArray.size() ? -1 : m_scoreArray[m_currentScoreIndex].record; }
      float GetCurrentScore(void) { return m_currentScoreIndex >= m_scoreArray.size() ? -1 : m_scoreArray[m_currentScoreIndex].score; }
...
class _EXPORT SpatialAllocator : public EnvModelProcess
   ...
   public:
      ...
      MapLayer* m_pIDULayer = nullptr;            // memory managed by EnvModel

      PtrArray<LAYER_INFO> m_layerInfoArray;
      ...
      UPDATE_OPERATOR m_updateOperator = UO_REPLACE;    // 0=replace, 1=OR
      Budget* m_pBudget;
   ...
   public:
      ...
      LAYER_INFO* SetLayerInfo(TargetContainer*, MapLayer*);
