...
class Preference
   ...
   public:
      ...
      ~Preference(void) { if (m_pMapExpr != nullptr) delete m_pMapExpr; }
...
class Constraint
   ...
   public:
      ...
      Constraint() : m_queryStr(), m_cType(CT_QUERY), m_pQuery(nullptr), m_pAlloc(nullptr) { }
...
class TargetContainer
   ...
   public:
      FDataObj* m_pTargetData = nullptr;      // input data table for target inputs
      ...
      ~TargetContainer() { if (m_pTargetData != nullptr) delete m_pTargetData; }
...
class Allocation : public TargetContainer
   ...
   public:
      ...
      Query* m_pExpandQuery = nullptr;

      CostItem* m_pCostItem = nullptr;
   ...
   public:
      ...
      AllocationSet* m_pAllocSet = nullptr;   // associated allocation set
...
class AllocationSet : public TargetContainer
   {
   public:
      SpatialAllocator* m_pSAModel = nullptr;
      CString m_field;
      ...
      MapExpr* m_pTargetMapExpr = nullptr;  // memory managed by?

      PtrArray< Allocation > m_allocationArray;

      FDataObj* m_pOutputData = nullptr;         // target and realized % areas for each allocation
      ...
      AllocationSet(SpatialAllocator* pSAModel, LPCTSTR name, LPCTSTR field)
         ...
         m_inUse(true), /*m_shuffleIDUs( true ),*/ m_pOutputData(nullptr), /* m_pOutputDataCum( nullptr ), */
...
class _EXPORT SpatialAllocator : public EnvModelProcess
   ...
   public:
      ...
      MapLayer* m_pMapLayer = nullptr;            // memory managed by EnvModel
      MapExprEngine* m_pMapExprEngine = nullptr;     // memory managed by EnvModel
      QueryEngine* m_pQueryEngine = nullptr;       // memory managed by EnvModel
   ...
   public:
      bool LoadXml(EnvContext*, LPCTSTR filename, PtrArray< AllocationSet >* pAllocSetArray = nullptr);
      bool LoadXml(LPCTSTR filename, PtrArray< AllocationSet >* pAllocSetArray = nullptr);
      ...
      FDataObj* m_pBuildFromFile = nullptr;
      CString m_update_from_file_name;
   protected:
      int SaveXml(FILE* fp);   // helper function

      bool m_shuffleIDUs;
      RandUniform* m_pRandUnif = nullptr;     // for shuffling IDUs   
      ...
      // data collection
      FDataObj* m_pBudgetData = nullptr;

      bool m_collectExpandStats;
      FDataObj* m_pExpandStats = nullptr;    // cols are size bins, rows are active allocations
