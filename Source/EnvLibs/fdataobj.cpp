...
float FDataObj::IGet( float x, float y, IMETHOD method )
   ...
   // Obtain interpolated y values for both the x0row and x1row
	  ...
	  float yval_x0row = ((y-y0)/(y1-y0))*(y0_x0row - y1_x0row)+y1_x0row;

	  float y0_x1row = Get(y0col,x1row);
	  float y1_x1row = Get(y1col,x1row);
	  float yval_x1row = ((y-y0)/(y1-y0))*(y0_x1row - y1_x1row)+y1_x1row;
...
int FDataObj::AppendCol( LPCTSTR label ) //, float *dataArray /*=NULL*/ )
   {
   float *dataArray = new float[ GetRowCount() ];
...
int FDataObj::InsertCol( int insertBefore, LPCTSTR label )
   {
   float *dataArray = new float[ GetRowCount() ];
...
int FDataObj::ReadAscii( LPCTSTR _fileName, TCHAR delimeter, BOOL showMsg )
   {
   CString fileName;
...
int FDataObj::ReadXls(LPCTSTR fileName)
   {
...
int FDataObj::WriteAscii( LPCTSTR fileName, TCHAR delimiter, int colWidth )
   {
